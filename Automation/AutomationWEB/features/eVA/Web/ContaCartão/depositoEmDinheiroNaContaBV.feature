Feature: Solicitar deposito em conta no site


  @CT1-DEPOSITO-CONTA-SITE
  ...
  Scenario: Solicitar deposito em conta (deposito pix)
    ...
    And validar conteudo (ballon 5) "Para cadastrar ou conferir suas chaves, você precisa ter o app banco BV no seu celular e:"
  ...
  @CT3-DEPOSITO-CONTA-SITE
  Scenario: Solicitar deposito em conta (deposito qrcode)
    Given que acessei o chatbot do eva (Web)
    When eu digitar a intencao "depositar dinheiro"
    And enviar a mensagem
    And clicar no botao "QR Code Pix"
    Then validar conteudo (ballon 5) "Para fazer um depósito por QR Code Pix, você precisa ter o app banco BV no seu celular e:"
  ...
  @CT4-DEPOSITO-CONTA-SITE
  Scenario: Solicitar deposito em conta (deposito ted)
    Given que acessei o chatbot do eva (Web)
    When eu digitar a intencao "depositar dinheiro"
    And enviar a mensagem
    And clicar no botao "TED"
    Then validar conteudo (ballon 5) "Para fazer um depósito por TED, você precisa ter o app banco BV no seu celular e:"
    And validar conteudo (ballon 5) "Ir na tela inicial e escolher a opção Depositar dinheiro aqui"
  ...
  @CT5-DEPOSITO-CONTA-SITE
  Scenario: Solicitar deposito em conta (deposito doc)
    Given que acessei o chatbot do eva (Web)
    When eu digitar a intencao "depositar dinheiro"
    And enviar a mensagem
    And clicar no botao "DOC"
    Then validar conteudo (ballon 5) "Para fazer um depósito por DOC, você precisa ter o app banco BV no seu celular e:"
    And validar conteudo (ballon 5) "Ir na tela inicial e escolher a opção Depositar dinheiro aqui"
    And validar conteudo (ballon 5) "Selecionar Por TED ou DOC"
    And validar conteudo (ballon 5) "Depois, é só copiar ou compartilhar os dados e fazer a transferência em outro banco."
