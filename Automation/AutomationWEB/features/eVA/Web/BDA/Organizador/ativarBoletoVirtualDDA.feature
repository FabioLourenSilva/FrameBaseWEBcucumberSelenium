Feature: Ativar boleto virtual (DDA) no site

  Scenario: Solicitar ativação do boleto virtual através das intenções
    Given que acessei o chatbot do eva (Web)
    When eu digitar a intencao "<intent>"
    And enviar a mensagem
    Then validar conteudo (ballon 3) "Para ativar o Boleto Digital (DDA), você precisa ter a conta digital no app BV."
...
#      | Ativar boleto digital                    |
  ...
  Scenario: Solicitar ativação do boleto virtual e conhecer o dda
    ...
    And validar conteudo (ballon 5) "Lá você acompanha seus gastos mensais pra se programar melhor e ainda pode pagar direto pelo app BV. 😉"
