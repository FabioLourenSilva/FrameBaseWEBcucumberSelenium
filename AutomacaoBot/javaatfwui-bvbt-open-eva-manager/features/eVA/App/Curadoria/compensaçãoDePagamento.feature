Feature: Compensação de pagamento no app
  ...
  Scenario: Solicitar compensação de pagamento da fatura do cartão bv (Saber mais sobre o limite)
    ...
    Then validar conteudo (ballon 7) "Consulte os limites do seu cartão de forma rápida na opção Meus Cartões."
    And validar conteudo (ballon 7) "O limite disponível é o valor que você ainda tem para fazer compras"
    And validar conteudo (ballon 7) "O limite total aparece logo abaixo do limite disponível e representa o valor do limite liberado para o seu cartão"
