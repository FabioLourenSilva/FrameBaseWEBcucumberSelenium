Feature: Como funciona (Organizador financeiro) no site

  Scenario: Perguntar como funciona o organizador através das intenções
    Given que acessei o chatbot do eva (Web)
    When eu digitar a intencao "<intent>"
    And enviar a mensagem
    Then validar conteudo (ballon 3) "O Organizador financeiro é mais um dos benefícios da sua conta digital no app BV."
    And validar conteudo (ballon 3) "Com ele, você visualiza, agenda e paga suas contas em um só lugar, sejam elas boletos digitais, parcelas do seu financiamento de veículos e faturas dos seus Cartões BV."
  ...
  Scenario: Perguntar o que é boleto virtual através das intenções
    ...
    Then validar conteudo (ballon 3) "Lá você acompanha seus gastos mensais pra se programar melhor e ainda pode pagar direto pelo app BV."
  ...
  Scenario: Perguntar como funciona o organizador de beneficios
    ...
    Then validar conteudo (ballon 5) "Antes de mais nada, queria dizer que o Organizador financeiro tem benefícios que eu também queria aproveitar, mas robôs não têm boletos pra pagar."
  ...
  Scenario: Perguntar como funciona o organizador de boleto digital DDA, até o transbordo
    ...
    And validar conteudo (ballon 5) "Para ativar o Boleto Digital (DDA), você precisa ter a conta digital no app BV"
  ...
  Scenario: Perguntar como funciona o organizador de organizar contas (Contas a pagar), até o transbordo
    ...
    And validar conteudo (ballon 7) "Acessar tela inicial do app BV"
  ...
  Scenario: Perguntar como funciona o organizador de organizar contas (Contas pagas), até o transbordo
    ...
    And validar conteudo (ballon 7) "tEm Contas pagas, você pode confirmar o que já foi pago e ainda saber o valor total que foi gasto. Para ter uma ideia mês a mês, é só selecionar a opção Filtrar. Quer um comprovante? É muito fácil! Mas eu te conto direitinho apenas se você precisar. 😉"
    ...
    And validar conteudo (ballon 9) "Ir em Organizar contas 👉 Contas pagas"
  ...
  Scenario: Perguntar como funciona o organizador de organizar contas (Desativar DDA), até o transbordo
    ...
    And validar conteudo (ballon 7) "Para desativar o Boleto Digital (DDA), você precisa ir no app BV e:"
    And validar conteudo (ballon 7) "Acessar a tela inicial"
    And validar conteudo (ballon 7) "Tocar em Organizar contas"
