Feature: Programar débito automático no app
  ...
  Scenario: Perguntar como programar o débito automatico através das intenções
    ...
    And validar conteudo (ballon 3) "Mas, pra te ajudar a organizar melhor suas contas e pagamentos, você pode ativar o Boleto Digital (DDA)."
    And validar conteudo (ballon 3) "Ele busca e centraliza todos os boletos digitais cadastrados no seu CPF, mesmo os de outras instituições, em um único lugar, o Organizador Financeiro BV."
  ...
  Scenario: Perguntar como programar o débito automatico (Conhecer o DDA)
    ...
    Then validar conteudo (ballon 7) "Para ativar o Boleto Digital (DDA), você precisa:"
