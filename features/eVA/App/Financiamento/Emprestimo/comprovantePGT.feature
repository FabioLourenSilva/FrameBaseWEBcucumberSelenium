Feature: Comprovante PGT APP
  ...
  Scenario: Solicitar comprovante de pagamento do saldo em conta (Transbordo e deixar para depois)
    ...
    And validar conteudo (ballon 5) "Certo! Para pegar um comprovante, é só:"
  ...
  Scenario: Solicitar comprovante de pagamento do cartão BV, não dando certo o procedimento (Transbordo e deixar para depois)
    ...
    Then validar conteudo (ballon 7) "Ah, tudo bem! Vou te contar um outro jeito de acessar o seu comprovante. É só: "
  ...
  Scenario: Solicitar comprovante de pagamento do cartão BV e conhecer o DDA
    ...
    And validar conteudo (ballon 9) "Para ativar o Boleto Digital (DDA), você precisa:"
...
#    And validar conteudo (ballon 9) "A partir dali, siga as instruções da tela e pronto!"
  ...
  Scenario: Solicitar comprovante de pagamento do pagamento de financiamento (Saber mais)
    ...
    And validar conteudo (ballon 5) "Para gerar um extrato detalhado com as informações do seu financiamento ou empréstimo, você precisa acessar o site e:"
    And validar conteudo (ballon 5) "Ir na página inicial da Minha BV"
    And validar conteudo (ballon 5) "Clique em Financiamento"
    ...
    And validar conteudo (ballon 6) "Ah, e essas informações também estão disponíveis aqui no app do BV, caso não queira o extrato em PDF."
  ...
  Scenario: Solicitar comprovante de pagamento do pagamento de financiamento (Deixar para depois)
    ...
    Examples:
      | button_comprovante      | button               | content                                               |
      | Pagamento financiamento | Falar com uma pessoa | atendimento                                           |
      | Pagamento financiamento | Deixar para depois   | Se eu puder te ajudar com outra coisa, me conta aqui. |
      | Pagamento empréstimo    | Falar com uma pessoa | atendimento                                           |
