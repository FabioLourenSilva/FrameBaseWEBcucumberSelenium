Feature: Portabilidade de salário (Web)

  @CT1-PORTABILIDADE-DE-SALARIO-WEB
  ...
  @CT2-PORTABILIDADE-DE-SALARIO-WEB
  Scenario: Solicitar portabilidade (Chave Pix)
    ...
    And validar conteudo (ballon 5) "Se você tem cadastro em outra instituição e agora quer ter sua chave Pix com a gente, é só pedir a sua portabilidade pelo app banco BV. Lá, é só você:"
...
#    diferente no web
  @CT3-PORTABILIDADE-DE-SALARIO-WEB
  Scenario: Solicitar portabilidade (Salário)
    ...
    Then validar conteudo (ballon 5) "Tenha em mãos o seu demonstrativo de pagamento ou informe de rendimentos. Depois, é só acessar o app BV no seu celular e:"
  ...
  @CT4-PORTABILIDADE-DE-SALARIO-WEB
  ...
  @CT5-PORTABILIDADE-DE-SALARIO-WEB
  Scenario: Solicitar portabilidade do salário (Não consigo autorizar celular)
    ...
    Then validar conteudo (ballon 7) "Se você já tem cadastro no app BV, precisa autorizar o seu celular e liberar todas as funções. Para isso, é só: "
    And validar conteudo (ballon 7) "Acessar o app com o seu CPF e senha"
    And validar conteudo (ballon 7) "Ir na tela inicial do app"
    And validar conteudo (ballon 7) "Tocar em Perfil, que fica no ícone no canto superior direito"
    And validar conteudo (ballon 7) "Escolher Configurações → Liberar todas as funções no menu App BV"

  @CT6-PORTABILIDADE-DE-SALARIO-WEB
  Scenario: Solicitar portabilidade do salário (Esqueci a senha)
    Given que acessei o chatbot do eva (Web)
    And eu digitar a intencao "Quero trazer meu salário pro BV"
    And enviar a mensagem
    And clicar no botao "Já tentei isso"
  ...
  @CT7-PORTABILIDADE-DE-SALARIO-WEB
  ...
  @CT8-PORTABILIDADE-DE-SALARIO-WEB
...
#    diferente na web
  @CT9-PORTABILIDADE-DE-SALARIO-WEB
  Scenario: Solicitar portabilidade do salário (Não recebi o valor - Menos de 10 dias úteis)
    ...
    Then validar conteudo (ballon 9) "Enquanto isso, você pode consultar a situação do seu pedido de portabilidade quando quiser pelo app BV. É só:"
    And validar conteudo (ballon 9) "Ir na tela inicial do app"
    And validar conteudo (ballon 9) "Tocar em Trazer meu salário"

  @CT10-PORTABILIDADE-DE-SALARIO-WEB
  ...
  @CT11-PORTABILIDADE-DE-SALARIO-WEB
  ...
  @CT12-PORTABILIDADE-DE-SALARIO-WEB
